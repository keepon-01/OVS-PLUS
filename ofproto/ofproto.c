...
//这个地方是选择流规则清除，我们可以看看有没有我们可以用到的地方
...
ofproto_init(const struct shash *iface_hints)
{
    ...
    size_t i;
    /* ofproto接口类注册，实际上整个程序就一个接口ofproto_dpif_class */
    ofproto_class_register(&ofproto_dpif_class);

    /* Make a local copy, since we don't own 'iface_hints' elements. */
    SHASH_FOR_EACH(node, iface_hints) {/*遍历每一个interface*/
    ...
    /* 目前的ofproto_class只有一个，那就是ofproto_dpif_class */
...
{
    size_t i;
    /* 这里实际调用的是ofproto_dpif_class的enumerate_type函数*/
    sset_clear(types);
    for (i = 0; i < n_ofproto_classes; i++) {
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    *ofprotop = NULL;
    ...
    class = ofproto_class_find__(datapath_type);/* 就一个ofproto_dpif_class */
    if (!class) {
    ...
    ofproto->ofproto_class = class;/* 记录下class */
    ...
    error = ofproto->ofproto_class->construct(ofproto);       /* 这里挺重要的一个历程步骤*/
    if (error) {
...
ofproto_type_run(const char *datapath_type)
{
    ...
    datapath_type = ofproto_normalize_type(datapath_type);
    /* 找到对应的ofproto_class,就一个，就是ofproto_dpif_class */
    class = ofproto_class_find__(datapath_type);
    /* 运行type_run */
...
ofproto_run(struct ofproto *p)
{
    int error;
    uint64_t new_seq;
    /* 对应ofproto_dpif_class的run函数 */
    error = p->ofproto_class->run(p);
    if (error && error != EAGAIN) {
        VLOG_ERR_RL(&rl, "%s: run failed (%s)", p->name, ovs_strerror(error));
    }

    /* 2.5版本中有run_rule_executes(p), tamen 说是执行rule配置到dp */
               ...
               struct netdev *netdev, const struct ofputil_phy_port *pp)
{
    ...
    ofport->created = time_msec(); /* 创建的时间, 这里是否可以利用起来呢 */
    ...
    OVS_REQUIRES(ofproto_mutex)
{
    ...
    if (!old_rule) {/* 如果流表项不存在 */
        ...
        if (table->n_flows >= table->max_flows) {
            if (!choose_rule_to_evict(table, &old_rule)) {                       /* 这里是在流表满的时候我们进行清楚 */
    ...
    ofm->temp_rule = NULL;
    /* rule中存储的是老规则，new_rule中存储的是新的规则*/
    ...
    OVS_NO_THREAD_SAFETY_ANALYSIS
{
    ...
    /* Allocate new rule. */
    rule = ofproto->ofproto_class->rule_alloc();/* 分配资源 */
    if (!rule) {/* 新规则分配失败 */
        cls_rule_destroy(cr);
    ...
    *CONST_CAST(uint8_t *, &rule->table_id) = table_id;
    rule->flags = flags & OFPUTIL_FF_STATE; /* 标记 */
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    struct oftable *table = &ofproto->tables[fm->table_id];/* 获得流表 */
    struct rule *rule;
...
replace_rule_start(struct ofproto *ofproto, struct ofproto_flow_mod *ofm,
                   struct rule *old_rule, struct rule *new_rule)
{
    struct oftable *table = &ofproto->tables[new_rule->table_id]; /* 获得流表 */
 ...
 * The deleted rules will become invisible to the lookups in the next version.       ?lookup
 * Destroys 'rules'. */
static void
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    error = reject_slave_controller(ofconn);/* 角色权限认证 */
    if (error) {
        return error;
    }
...
/* 这里本来是将flow mod下发到datapath， 但是没有run_rule_executes(ofproto)(将真正的数据下发到datapath)， 可能版本不一样，这里不安装到dp？ */
static enum ofperr
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    error = ofproto_flow_mod_init(ofproto, &ofm, fm, NULL);       /* 重要的步骤1 */
    if (error) {
        return error;
    }
    ...
    OVS_REQUIRES(ofproto_mutex)
{
    ...
    switch (ofm->command) {
    case OFPFC_ADD:/* 添加flow MOD */                       /* 实际我这里最想拿到的就是flow mod里的添加类别 */
        error = add_flow_start(ofproto, ofm);
        break;
    case OFPFC_MODIFY:/* 修改flow MOD */
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    case OFPTYPE_FLOW_MOD:
        return handle_flow_mod(ofconn, oh);   /* 把这个特别出来，特别出一个缓存出来 */
...
/*  这里需要我们看下，这个地方是安装flow mod消息在流表满的情况下执行的方法   skrskr */
static bool
 ...
 *
 * 这个地方就是我们关心的，一个基于可替换流规则的策略，可以看下  skrskr
 * */
static void
