...
{
    if (!guarded_list_push_back(&ofproto->ams, &am->list_node, 1024)) {    /* 如果超过1024个，就认为是packet in消息队列溢出了 */
...
type_run(const char *type)
{
    ...
    /* 关于all_dpif_backers,只有调用construct函数的时候才会有内容加入 */
...
open_dpif_backer(const char *type, struct dpif_backer **backerp)
{
    ...
    const char *name;
    int error;
    /* 关于all_dpif_backers,仅仅当调用了construct函数，才会往
     * all_dpif_backers中添加内容
     */
    backer = shash_find_data(&all_dpif_backers, type);
    if (backer) {
        backer->refcount++;
    ...
    *backerp = backer;
    ...
    if (backer->recv_set_enable) {/* 如果允许接收数据 */
        udpif_set_threads(backer->udpif, n_handlers, n_revalidators);
    }
    /* 获得版本信息 */
...
construct(struct ofproto *ofproto_)
{
    ...
    error = open_dpif_backer(ofproto->up.type, &ofproto->backer);/* 也是重要的历程的方法*/
    if (error) {
        return error;
    }
    ...
    /* 为什么端口不存在，就不进行创建工作 */
    SHASH_FOR_EACH_SAFE (node, next, &init_ofp_ports) {/*  遍历端口信息 */
 ...
 * Any changes are restored before returning.
 *
 * 这个地方可能就是在用户态的流表classfier里面进行查找
 * */
struct rule_dpif *
...
/*
 * 我们知道 struct ofproto_class 代表openflow 交换机的接口，每一个openflow交换机实例都须要实现这个接口，
 * 这里的实例是 ofproto_dpif_class，它引出一个交换机全部的资源。
 * */
