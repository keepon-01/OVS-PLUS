...
/* 初始化ofproto库。这仅需要执行一次，但配置设置之后它必须要做的。
 * 如果已经出现了初始化，bridge_init_ofproto（）立即返回。 */
...
bridge_reconfigure(const struct ovsrec_open_vswitch *ovs_cfg)
{
    ...
    //空闲过期时间这里感觉也有涉及，和ofproto的是什么关系呢
    /*  如果没有配置，就使用默认的配置 */
     ...
     * 这里是vswitched中一个很重要的函数片段，创建bridge就在这里实现
     * 在前面的add_del_bridges
     *     */
        ...
        for (i = 0; i < port_cfg->n_interfaces; i++) {
            ...
            if ((requested_ofp_port != OFPP_NONE) == with_requested_port) {
                ...
                if (!iface) {
                    /* 创建端口信息
                     *
                     * 这里有一点需要注意的，iface_create里面实际调用了
                     * */
...
add_del_bridges(const struct ovsrec_open_vswitch *cfg)
{
    ...
    /* 遍历旧的网桥，如果发现在新配置中找不到的话，说明此网桥被移除了 */
    HMAP_FOR_EACH_SAFE (br, next, node, &all_bridges) {
        br->cfg = shash_find_data(&new_br, br->name);
        if (!br->cfg || strcmp(br->type, ofproto_normalize_type(
                                   br->cfg->datapath_type))) {
            bridge_destroy(br, true);/* 析构网桥 */
        }
    }
             ...
             const struct ovsrec_port *port_cfg)
{
    ...
    port = port_lookup(br, port_cfg->name);/* 寻找端口信息 */
    if (!port) {
    ...
    iface->netdev = netdev;  /* 一个接口和一个netdev相关联 */
 ...
 * */
void
bridge_run(void)//主要对网包进行完整处理过程。包括完成必要的配置更新
{
    static struct ovsrec_open_vswitch null_cfg;
    const struct ovsrec_open_vswitch *cfg;

    ovsrec_open_vswitch_init(&null_cfg);/* 创建一个完全的空配置 */

    ovsdb_idl_run(idl);//处理了一批从’IDL’数据库服务器的消息。这可能会导致IDL的内容发生变化。客户端可以检查与ovsdb_idl_get_seqno（）

    if_notifier_run();

    if (ovsdb_idl_is_lock_contended(idl)) {  /* 数据库已经被锁住了 */
        ...
        return;
               ...
               || !ovsdb_idl_has_ever_connected(idl)) { /* 如果没有从ovsdb中获得数据*/
        /* Returns if not holding the lock or not done retrieving db
         * contents. */
        return;
    }
    cfg = ovsrec_open_vswitch_first(idl);/* 获取ovsdb中的数据 */
     ...
     *
     * 如果flow-restore-wait是FALSE，我们不应该检测数据中它的值
     * */
    ...
    if (ovsdb_idl_get_seqno(idl) != idl_seqno ||
        ...
        idl_seqno = ovsdb_idl_get_seqno(idl);
        txn = ovsdb_idl_txn_create(idl);
        bridge_reconfigure(cfg ? cfg : &null_cfg);/* 根据配置信息，重新构建网桥 */
...
bridge_del_ports(struct bridge *br, const struct shash *wanted_ports)
{
    ...
    HMAP_FOR_EACH_SAFE (port, next, hmap_node, &br->ports) {
        ...
        if (!port->cfg) {
            port_destroy(port);/* 端口没有找到就要移除 */
        } else {
            /*
             * 端口找到了，可能里面的接口信息发生了变化
             * 新的接口信息存储在port_cfg中
             * */
    ...
    SHASH_FOR_EACH (port_node, wanted_ports) {
        ...
        for (i = 0; i < port_rec->n_interfaces; i++) {
            ...
            if (iface) {/* 找到了的话，更新cfg, type, netdev_type等信息  */
                iface->cfg = cfg;
                iface->type = type;
            } else {
    ...
    *oc = (struct ofproto_controller) {
    ...
    for (size_t i = 0; i < n_controllers; i++) {
        ...
        if (daemon_should_self_confine()
            && (!strncmp(c->target, "punix:", 6) /* 被动监听 */
            || !strncmp(c->target, "unix:", 5))) {/* 发起主动链接 */
            static struct vlog_rate_limit rl = VLOG_RATE_LIMIT_INIT(1, 5);
            char *whitelist;
