...
{   VLOG_INFO("----------------mxc----------------");
    ...
    ovs_cmdl_proctitle_init(argc, argv);//复制出输入的参数列表到新的存储中，让argv指向这块内存
    service_start(&argc, &argv);//注册回调和服务管理器出现故障错误时候操作的配置
    remote = parse_options(argc, argv, &unixctl_path);//解析参数，其中unixctl_path存储着sock,作为接收外部控制命令的渠道，而remote存储连接到ovsdb信息，也就是连接到配置数据库的sock名
    fatal_ignore_sigpipe();

    daemonize_start(true);//开始守候进程模式
    ...
    bridge_init(remote);//初始化这个bridge
    free(remote);
    ...
    //main函数里面的一直循环的循环体
    while (!exiting) {
        memory_run();//运行内存监视器，客户端调用memory_should_report()。此函数以及该模块的接口的剩余部分，仅被一个线程调用。
        ...
        bridge_run();//其中bridge_run就是初始化数据库中已经创建的虚拟网桥 ////处理与controller的交互，ovs-ofctl命令
        unixctl_server_run(unixctl);//从unixctl指定的server获取ovs-appctl发送的命令并执行 ////处理ovs-vsctl命令
        netdev_run();//在内核中添加虚拟网卡，也就是初始化数据库中的虚拟网卡
        ...
        poll_block();//阻塞知道被poll_fd_wait()注册的事件发生或者超时
